...
$cache_options = [
    xPDO::OPT_CACHE_KEY => 'default/file_snippets/mfilter2/' . $modx->context->key . '/',
];
...
if (!empty($elementName) && $element = $modx->getObject('modSnippet', array('name' => $elementName))) {
    ...
    $cache_name = "ids-" . $modx->resource->id;
    $tmp = $modx->cacheManager->get($cache_name, $cache_options);
    if (empty($tmp)) {
        $tmp = $element->process($params);
        $modx->cacheManager->set($cache_name, $tmp, 0, $cache_options);
    }
...
if (!empty($scriptProperties['tpls'])) {
    $tmp = isset($_REQUEST['tpl']) ? (int) $_REQUEST['tpl'] : 0;
...
if (!empty($sort)) {
    ...
    $output['sort'] = $sort == $start_sort ? '' : $sort;
}
...
$config = array_merge($config, array(
    ...
    'key' => $hash,
    'pageId' => !empty($pageId) ? (int) $pageId : $modx->resource->id,
...
$cache_prefix = $mSearch2->config['cache_prefix'] . 'settings/' . md5(json_encode($_GET));
$modx->cacheManager->set($cache_prefix . '/config',     $config);
$modx->cacheManager->set($cache_prefix . '/properties', $savedProperties);
